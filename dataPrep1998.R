...
## Extract 1998 well-being data
	...
	query <- paste("SELECT ", paste(lsTags$Variable, collapse=","),",", 
				   ...
				   ",SCHOOLNO, STATE, SEX, INDIG, ANU3DAD, ANU3MUM,
				   INTDAT00, DOB FROM LSAY1998")
	...
	d1998$age <- as.Date(d1998$INTDAT00, "%d/%m/%y") - as.Date(d1998$dob, "%d/%m/%y")
...
dLong98$post06 <- ifelse(dLong98$trend > 3,1,0)
dLong98$post03 <- ifelse(dLong98$trend > 6,1,0)
#add cohort variable
dLong98$cohort <- 'y98'
...
MI <- amelia(dLong98,m = 5, p2s = 1, idvars = c('WT', 'id', 'schoolno', 'post03', 'post06', 'cohort', 'state'),
			 noms = c("EGP", "sex", "indig") )
a98 <- imputationList(MI$imputations)
#save to Rdata
save(a98, file = "Cohort98Data.RData")
rm(list=c("dLong98", "d1998", "lsTags", "weightTags", "F1998", "m1998"))
# Wrap in srvey package
dclust <- svydesign(ids = ~id+schoolno, weights = ~WT, data = a98, nest=TRUE)
rm(a)
