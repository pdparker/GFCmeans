...
## Extract 1995 well-being data
	...
	d1995$age <- as.Date(d1995$INTDAT97, "%d/%m/%y") - as.Date(d1995$dob, "%d/%m/%y")
	# translate to EGP
	...
	d1995$HISEI <- apply(d1995[,c("ISEI_MUM", "ISEI_F")],1, max, na.rm = TRUE) %>% 
		recode("-Inf = NA") 
	d1995$EGP <- sesConvert(d1995$HISEI, fromFormat = "ISEI", toFormat = "EGP", FUN = Mode)
...
## Survey package - Imputation and Replicant Weights
	...
	# Stack data
	#Do not include the first wave as occured at age 15
	dLong95 <- rbind.data.frame(d1995[,c(2,12,22,32,42,52,62,72,82,92,102,112,122,131:N)],
	...
	dLong95$trend <- rep(1:9, each=(nrow(d1995)))
	#add treatment variable
	dLong95$post06 <- ifelse(dLong95$trend > 3,1,0)
	dLong95$post03 <- ifelse(dLong95$trend > 6,1,0)	#add cohort variable
	#add treatment variable
	dLong95$cohort <- 'y95'
	...
	#save to Rdata	#Multiple Imputations of data holes
	MI <- amelia(dLong95,m = 5, p2s = 1, idvars = c('WT', 'id', 'post03', 'post06', 'cohort', 'state'),
				 noms = c("EGP", "sex", "indig") )
	a95 <- imputationList(MI$imputations)
	save(a95, file = "Cohort95Data.RData")
	rm(list=c("dLong95", "d1995", "lsTags", "weightTags", "F1995", "m1995"))
	# Wrap in srvey package
	dclust95 <- svydesign(ids = ~id+schno, weights = ~WT, data = a95, nest=TRUE)
	rm(a)
